...
void RealTimeClock::begin(void) { //Set up our clock
    ...
    TACCTL0 = CCIE;             //  CCR0 interupt activated
#ifdef RTCSUBSECONDS
    TACCR0 = 128-1;             // 128 ticks is 1/256th of a second
    TACTL = TASSEL_1 | ID_0 | MC_1;  // Clock for TIMER 1 = ACLK, No division, up mode
#else
    TACCR0=4096-1;                     // we are dividing clock by 8, so 4096 ticks = 1 second
    TACTL = TASSEL_1 | ID_3 | MC_1;	// Clock for TIMER 1 = ACLK, By 8 division, up mode
#endif
};
